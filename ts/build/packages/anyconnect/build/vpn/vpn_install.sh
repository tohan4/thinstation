...
LEGACY_INSTPREFIX=/tmp
LEGACY_BINDIR=${LEGACY_INSTPREFIX}/bin
LEGACY_UNINST=${LEGACY_BINDIR}/vpn_uninstall.sh

TARROOT="vpn"
INSTPREFIX=$INSTALLDIR/opt/cisco/anyconnect
ROOTCERTSTORE=$INSTALLDIR/opt/.cisco/certificates/ca
...
LOGFNAME=`date "+anyconnect-linux-3.1.14018-k9-%H%M%S%d%m%Y.log"`
...
if [ "${TEMPDIR}" != "." ]; then
  ...
  NEWTEMP=./packages/anyconnect/build/vpn
fi
...
#Added for ThinStation 
INITD=$INSTALLDIR/etc/init.d
